...
```text
가장 기억에 남는 주체와 관계 읽기 → 완전한 배경과 잉여 세부 삭제 → 소수의 망점 형태와 부분 구조로 압축 → 밀도와 인쇄 층으로 골격 형성 → 원본에서 온 소수의 컬러 선 추가 → 여백, 가벼운 글자, 편집 질서로 통합 구도 완성
```

## 완성작의 특징

- 검정 또는 짙은 망점, 입자, 밀도 변화가 주요 골격을 만들며 주체는 추출되어도 식별됩니다.
- 컬러 궤적, 고리, 둘러 그린 표시, 안내선은 장식이 아니라 감정, 방향, 공기, 시간에 기여합니다.
- 아이보리, 웜 화이트, 매우 옅은 바탕에 넓은 숨 쉴 공간을 남겨 소수의 표식에 긴장감을 줍니다.
- 글자는 선을 따르거나 망점 주체 가까이, 또는 여백 가장자리에 절제되고 가볍게 놓입니다.
- 사실적 그림, 의미 없는 장식, 요소 축적, 값싼 필터, 카툰, 템플릿을 피합니다.

미적 제약은 [원본 프롬프트](references/original-prompt/zh-CN.md)에만 있습니다. Skill과 실행 어댑터는 납품 변수만 처리합니다. [Skill](SKILL.md) · [영문 실행 어댑터](references/xxd-panel-104-prompt.en.md)
...
### Xiaoxiaodong 총괄 · 장군 총지휘 Skill · CNY 104

CNY 104을 한 번 결제하면 이 시리즈의 장군 총지휘 Skill(`xxd-panel-all`)을 사용할 수 있습니다. 전체 병사 Skills의 통합 관리, 추천, 지정 파견, 일괄 조정을 지원합니다. WeChat 메시지에 “장군 총지휘 Skill”이라고 적어 주세요.
