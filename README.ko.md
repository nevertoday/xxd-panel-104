...
# 🦁 XXD Panel 104｜망점 선의 기록

### 사진을 망점 형태, 컬러 선, 고요한 여백으로 다시 편집합니다
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-05.png" alt="XXD Panel 104 Sample 5"></td>
    <td width="50%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 104 Sample 6"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 104 Sample 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 104 Sample 8"></td>
  </tr>
</table>
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 104 additional top-bottom sample 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 104 additional top-bottom sample 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 104 additional top-bottom sample 3"></td>
    <td width="50%"><img src="./assets/examples/sample-12.png" alt="XXD Panel 104 additional top-bottom sample 4"></td>
  </tr>
</table>
...
위 여덟 작품은 16:9 가로형 좌우 구성 네 장과 3:4 세로형 상하 구성 네 장입니다. 모두 Panel 104이 자체 원본 프롬프트에 따라 독립적으로 생성했으며 다른 번호의 작품을 재사용하지 않았습니다. 예시 문구는 영어로 자동 생성했습니다.

<!-- xxd-human-intro:start -->
## 이 스타일이 해결하는 문제

사진을 디자인으로 다시 말하고 싶지만 장식을 쌓고 싶지 않을 때, **Panel 104**는 현실 사진을 유지하고 가장 기억에 남는 주체를 망점 형태로 압축합니다. 소수의 컬러 선, 넓은 옅은 바탕, 가벼운 편집 주석이 인쇄 질감과 고요한 시간감을 가진 질서를 만듭니다.

### 이런 경우에 적합합니다

- 기억할 가치가 있는 주체, 윤곽, 자세, 관계를 남기면서 방해되는 배경을 줄이고 싶을 때
- 실험적인 인쇄 감각은 원하지만 그래픽 요소가 어수선해지는 것은 피하고 싶을 때
- 시각적 무게중심, 밀도, 간격으로 여백을 공허하지 않고 의도적으로 만들고 싶을 때
- 전시 포스터, 편집 그래픽, 문화 표지, 디자인 전용 이미지, 여러 비율, 벽지, 폴더 일괄 처리

### 빠른 시작

> 이 이미지를 XXD Panel 104로 처리하고 가장 알맞은 구도와 크기를 먼저 추천해 주세요.
...
| 확인할 점 | 이 스타일의 답 |
|---|---|
| **결과물** | 망점 인쇄, 컬러 선, 예술적 여백, 미묘한 선적 절제가 함께 작동하는 편집 포스터 |
| **대표 특징** | 짙은 망점 골격, 밀도 변화, 소수의 컬러 궤적, 옅은 바탕, 가벼운 주석, 통합 구도 |
| **입력 존중 방식** | 정체성, 자세, 서사 관계를 유지하고 색은 사진에서 가져오며 기억점을 강화하기 위해서만 줄임 |
| **활용처** | 예술 전시, 실험 편집, 문화 포스터, 표지, 디자인 전용 이미지, 여러 비율, 벽지 |
...
```bash
npx skills add https://github.com/nevertoday/xxd-panel-104 --skill xxd-panel-104
```
...
Claude Code 사용자는 같은 폴더를 다음 위치에 연결할 수 있습니다: `~/.claude/skills/xxd-panel-104`. 설치 후 에이전트 세션을 다시 시작하세요.
...
전체 사양: [Skill 워크플로](SKILL.md) · [원본 스타일 자료](references/original-prompt/zh-CN.md) · [영문 런타임 어댑터](references/xxd-panel-104-prompt.en.md) · [중문 런타임 어댑터](references/xxd-panel-104-prompt.zh-CN.md)
...
**특징:** 망점 인쇄 · 컬러 선형 개입 · 통합 구도 · 예술적 여백 · 미묘한 선적 감각 · 편집 주석
...
이미지 폴더를 직접 전달하면 이 병사 Skill이 같은 Panel 104의 원본 미학으로 일괄 처리합니다. 일반 래스터 형식을 기본적으로 재귀 탐색하고 수량을 먼저 알린 뒤 모드·크기·텍스트·언어를 한 번만 확인합니다. 각 이미지는 내용이나 문구가 섞이지 않도록 독립 생성하며 전체 배치는 하나의 평면 작업 디렉터리만 사용합니다. 개별 실패는 기록하고 이후 이미지를 조용히 누락하지 않습니다.
...
/xxd-panel-104 photo.jpg --mode top-bottom --size auto --text prompt --locale ko-KR

# 좌우 비교: 3:2 가로형, 글자 없음
/xxd-panel-104 photo.jpg --mode left-right --size 3:2 --text none

# 디자인 전용: 9:16 휴대폰 세로형, 한국어 문구
/xxd-panel-104 photo.jpg --mode design-only --size 9:16 --text prompt --locale ko-KR
...
/xxd-panel-104 photo.jpg --mode design-only --size 3:4 --text prompt --locale ko-KR

# 정확한 문구를 재작성·번역·제목 추가 없이 사용
/xxd-panel-104 photo.jpg --mode design-only --size 3:4 --copy "빛을 이곳에 남겨 두기" --locale ko-KR

# 2개 모드 × 3개 크기로 독립적인 완성 구도 6개 생성
/xxd-panel-104 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ko-KR

# 폴더 전체에 Panel 104 적용, 공통 설정은 한 번만 확정
/xxd-panel-104 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ko-KR
...
현재 XXD Panel 시리즈는 001–112까지 업데이트되었으며, 각 Panel은 독립된 원본 프롬프트와 미적 논리를 유지합니다. 아래 표는 이 프로젝트 공개 시점의 과거 목록으로 001–104을 연속해서 나열하며, 현재 프로젝트는 굵게 표시합니다.
