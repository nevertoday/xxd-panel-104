<p align="center"><img src="./assets/banner.svg" alt="XXD Panel 104 プロジェクトバナー" width="1200"></p>
...
`sample-01`–`sample-04` は、検証可能なオリジナル公開作例のために確保しています。上の8点は、16:9の横長・左右構成4点と、3:4の縦長・上下構成4点です。すべて Panel 104 が自身の原文プロンプトに基づいて個別に生成したもので、別番号の作品は流用していません。作例のコピーは英語で自動生成しています。
...
```text
最も記憶に残る主体と関係を読む → 完全な背景と余分な細部を削る → 少数の網点形体と部分構造へ圧縮 → 密度と印刷層で骨格をつくる → 元写真由来の少数の色線を加える → 余白、軽い文字、版面秩序で全体構成を完成
```

## 完成作の特徴

- 黒または濃色の網点、粒子、密度変化が骨格をつくり、主体は抽出されても識別できます。
- 色線、軌跡、囲み、環状線は装飾ではなく、感情、方向、空気、時間に役立てます。
- アイボリー、暖白、ごく淡い背景に広い呼吸空間を残し、少数の印に緊張感を与えます。
- 文字は線に沿い、網点主体に寄り、または余白の縁に軽やかに置かれます。
- 写実、意味のない装飾、要素の堆積、安価なフィルター、カートゥーン、テンプレートを避けます。

美的制約は[原文プロンプト](references/original-prompt/zh-CN.md)だけにあります。Skill と実行アダプターは納品変数のみを扱います。 [Skill](SKILL.md) · [英語実行アダプター](references/xxd-panel-104-prompt.en.md)
...
| **[xxd-panel-104](https://github.com/nevertoday/xxd-panel-104)** | 網点禅意誌 · 網点印刷 · 色線の介入 · 全体構成 · 芸術的余白 · 繊細な禅意 |
...
### Xiaoxiaodong 総控 · 将軍総指揮 Skill · CNY 104

CNY 104 の一回払いで、このシリーズの将軍総指揮 Skill（`xxd-panel-all`）を利用できます。全兵士 Skills の統括、推薦、指名派遣、一括調整に対応します。WeChat では「将軍総指揮 Skill」と記載してください。
...
| **兵士級** | `xxd-panel-NNN`（現在 001–104） | 各番号が固有の原文プロンプトと美学だけを実行し、将軍から渡された一つの仕事を完成させます。 |
