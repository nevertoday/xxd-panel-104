...
上の8点は、16:9の横長・左右構成4点と、3:4の縦長・上下構成4点です。すべて Panel 104 が自身の原文プロンプトに基づいて個別に生成したもので、別番号の作品は流用していません。作例のコピーは英語で自動生成しています。

<!-- xxd-human-intro:start -->
## このスタイルが解決すること

写真を再デザインしたいけれど装飾を積み上げたくないとき、**Panel 104** は現実の写真を残し、最も記憶に残る主体を網点形体へ凝縮します。少数の色線、広い淡色面、軽やかな編集注記が、印刷の質感と静かな時間感覚をもつ秩序をつくります。

### 向いている用途

- 記憶に残る主体、輪郭、姿勢、関係を保ちながら、邪魔な背景を減らしたい場合
- 実験的な印刷感が欲しい一方、図形を雑然と積み上げたくない場合
- 視覚的重心、疎密、間隔により、空白を空虚ではなく意図的に見せたい場合
- 展覧会ポスター、編集グラフィック、文化系表紙、デザイン単体、複数比率、壁紙、フォルダ一括処理

### すぐ試す

> この画像を XXD Panel 104 で処理し、最適な構図とサイズを先に提案してください。
...
| 確認したいこと | このスタイルの答え |
|---|---|
| **得られるもの** | 網点印刷、色線、芸術的余白、繊細な禅意が一体となった編集ポスター |
| **識別しやすい特徴** | 濃色の網点骨格、密度変化、少数の色線、淡い背景、軽い注記、統合構図 |
| **入力の尊重** | 同一性、姿勢、物語関係を保ち、色は写真から取り、削減は記憶点を強めるために行う |
| **用途** | アート展、実験編集、文化ポスター、表紙、デザイン単体、複数比率、壁紙 |
...
Claude Code では同じフォルダを次へリンクできます： `~/.claude/skills/xxd-panel-104`. インストール後に Agent セッションを再起動してください。
...
完全仕様: [Skill ワークフロー](SKILL.md) · [原始スタイル資料](references/original-prompt/zh-CN.md) · [英語ランタイムアダプター](references/xxd-panel-104-prompt.en.md) · [中国語ランタイムアダプター](references/xxd-panel-104-prompt.zh-CN.md)
...
**特徴：** 網点印刷 · 色線の介入 · 全体構成 · 芸術的余白 · 繊細な禅意 · 編集注記
...
画像フォルダーを直接渡すと、この兵士 Skill が同じ Panel 104 の原文美学で一括処理します。一般的なラスター形式を既定で再帰検索し、件数を報告してから、モード・サイズ・文字・言語を一度だけ確認します。各画像は内容や文言を混ぜず独立生成し、バッチ全体は一つのフラットなタスクフォルダーへ保存します。個別の失敗は記録し、後続画像を黙って省略しません。
...
/xxd-panel-104 photo.jpg --mode top-bottom --size auto --text prompt --locale ja-JP
...
/xxd-panel-104 photo.jpg --mode design-only --size 3:4 --text prompt --locale ja-JP

# 正確な文言を、書き換え・翻訳・タイトル追加なしで使用
/xxd-panel-104 photo.jpg --mode design-only --size 3:4 --copy "光をここに残す" --locale ja-JP

# 2モード × 3サイズで独立した完成構図を6枚生成
/xxd-panel-104 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ja-JP

# フォルダー全体へ Panel 104 を適用し、共通設定は一度だけ解決
/xxd-panel-104 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ja-JP
...
XXD Panel の現行シリーズは 001〜112 まで更新され、各 Panel は独立した原文プロンプトと美的ロジックを保持しています。以下は本プロジェクト公開時点の履歴カタログで、001〜104 を連続して掲載し、現在のプロジェクトを太字で示します。
