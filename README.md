<p align="center"><img src="./assets/banner.svg" alt="XXD Panel 104 项目横幅" width="1200"></p>

<div align="center">

# 🦁 XXD Panel 104｜网点禅意志

### 把照片提炼成网点形体、彩色线性与安静留白
...
`sample-01`–`sample-04` 保留给未来可核验的原始发布样张。以上八张包括四张 16:9 横版左右样张和四张 3:4 竖版上下样张，均由 Panel 104 依据自己的原始提示词独立生成，不借用其他编号作品。样张使用英文智能配字。
...
- **内容有故事，但复杂背景抢走了注意力：** 提炼最值得保留的主体、轮廓和关系，用网点层次重新叙述。
- **想要实验印刷感，又不希望画面杂乱：** 以网点形体做骨架，彩线只承担方向、情绪和时间感。
- **需要安静高级的留白，而不是空洞：** 通过视觉重心、疏密节奏和版式秩序，让空白本身产生张力。
- **需要灵活交付：** 适合展览海报、编辑视觉、艺术封面、纯设计图、多比例成品、四端壁纸和目录批量处理。

### 快速开始

发送一张图片给 Agent，并说明：

> 用 XXD Panel 104 帮我处理这张图，先推荐最合适的构图和尺寸。
...
**关键词：** 网点印刷 · 彩色线性介入 · 整体构成 · 艺术留白 · 微妙禅意 · 编辑注释

<!-- xxd-panel-benefit:start -->
## 快速判断：XXD Panel 104 适合你吗？

| 你关心的问题 | 这套风格给你的回答 |
|---|---|
| **你会得到什么** | 一张以网点印刷、彩色线性介入、艺术留白和微妙禅意共同成立的编辑海报 |
| **一眼可辨的特点** | 深色网点骨架、疏密变化、少量彩线、浅色背景、轻巧注释与整体构成 |
| **它如何尊重你的输入** | 保留主体身份、姿态和叙事关系，色彩来自原图，删减只为突出记忆点 |
| **可以用在哪里** | 艺术展览、实验编辑、文化海报、封面、纯设计图、多比例成品和四端壁纸 |
<!-- xxd-panel-benefit:end -->

## 它如何把你的输入变成独特成品

```text
辨认最有记忆点的主体与关系 → 删除完整背景和冗余细节 → 压缩成少量网点形体与局部结构 → 用疏密和印刷层次建立骨架 → 引入少量原图来源的彩线 → 以留白、轻巧文字和版式秩序完成整体构图
```

## 成品中最容易识别的风格特点

- 黑色或深色网点、半调颗粒与密度变化构成主要骨架，主体被提炼但仍可识别。
- 彩色线条、轨迹、圈画或环形线只服务于情绪、方向、空气感和时间感。
- 米白、暖白或极浅背景保留大面积呼吸空间，让少量图形在空白中获得张力。
- 文字沿线条、贴近网点或落在留白边缘，以克制轻巧的方式进入整体。
- 拒绝写实插画、无意义装饰、元素堆积、廉价滤镜、卡通感和模板化版式。

原始审美约束与拒绝项只存在于[原始提示词](references/original-prompt/zh-CN.md)；Skill 与运行适配器只处理本次交付变量。 [Skill 工作流](SKILL.md) · [英文运行适配器](references/xxd-panel-104-prompt.en.md)
...
直接传入一个图片目录时，这个士兵 Skill 会自动批量处理目录中的图片，并让每张图片都使用 Panel 104 的同一套原始审美。默认递归扫描常见位图，先报告数量，再统一确认一次模式、尺寸、文字和语言；每张图片独立生成，互不借用内容或文案。整批仍然只创建一个扁平任务目录，单张失败会被记录，但不会静默跳过后续图片。
...
/xxd-panel-104 photo.jpg --mode top-bottom --size auto --text prompt --locale zh-CN
...
# 使用准确文案，不改写、不翻译、不补标题
/xxd-panel-104 photo.jpg --mode design-only --size 3:4 --copy "让风经过这里" --locale zh-CN

# 两种模式 × 三种尺寸，共生成六张独立完整构图
/xxd-panel-104 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale zh-CN

# 整个目录统一使用 Panel 104；公共设置只解析一次
/xxd-panel-104 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale zh-CN
...
Claude Code 用户可把同一文件夹链接到 `~/.claude/skills/xxd-panel-104`. 安装后请重启 Agent 会话。
...
完整规格: [Skill 工作流](SKILL.md) · [原始风格档案](references/original-prompt/zh-CN.md) · [英文运行适配器](references/xxd-panel-104-prompt.en.md) · [中文运行适配器](references/xxd-panel-104-prompt.zh-CN.md)
...
### 小小东总控 · 将军总指挥 Skill · 104 元

一次支付 104 元获得本套将军总指挥 Skill（`xxd-panel-all`），用于主控、推荐、点将和批量调度全部士兵 Skills。扫码后请备注“将军总指挥 Skill”。
...
| **士兵级** | `xxd-panel-NNN`（当前 001–104） | 每个编号只执行自己独立的原始提示词与审美，把将军派发的单个任务完成为成品。 |
